...
use crate::{
    error::ApiError,
    request::RequestId,
    routes::identity::{parse_id, simple_error},
};
...
pub(super) async fn update_role(
    ...
    let Some(id) = parse_id(&id).and_then(|id| i32::try_from(id).ok()) else {
        return Ok(simple_error(StatusCode::BAD_REQUEST, "Invalid account"));
    };
    let Some(role) = body
        .get("role")
        .and_then(Value::as_str)
        .map(str::to_ascii_lowercase)
        .filter(|role| matches!(role.as_str(), "user" | "moderator" | "developer" | "admin"))
    else {
        return Ok(simple_error(StatusCode::BAD_REQUEST, "Invalid role"));
    };
    let row = state
        .database
        .one_json(
    ...
    Ok(match row {
        ...
        None => simple_error(StatusCode::NOT_FOUND, "Account not found"),
    })
}
