...
#[derive(Clone, Copy, Debug, Eq, PartialEq, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct RankedPriorityReserveSnapshot {
...
pub fn calculate_ranked_priority_reserve_calls(
    due: u64,
    observed_peak: u64,
    hourly_floor: u64,
    calls_per_match: u64,
...
pub fn calculate_background_match_allowance(
    usable_calls: u64,
    reserve_calls: u64,
    worst_case_calls_per_match: u64,
) -> u64 {
    usable_calls.saturating_sub(reserve_calls) / worst_case_calls_per_match.max(1)
}

pub async fn ranked_priority_reserve_snapshot(
    database: &Database,
) -> Result<RankedPriorityReserveSnapshot, DatabaseError> {
    ...
    Ok(calculate_ranked_priority_reserve_calls(
        value_u64(row.get("pending_ranked_matches")),
        value_u64(row.get("observed_hourly_peak")),
        env_u64("RANKED_PRIORITY_MAX_MATCHES_PER_HOUR", 75),
        env_u64("RANKED_PRIORITY_CALLS_PER_MATCH", 13),
        env_u64("RANKED_PRIORITY_DISCOVERY_CALLS", 1),
    ))
}
